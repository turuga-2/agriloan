...
<script>
    ...
    $('#applicationDateFilter').datepicker({
        format: 'yyyy-mm-dd',
        autoclose: true,
        todayHighlight: true,
        endDate: new Date() // Disable picking future dates
    });

    // Initialize DataTable
    var table = $('#loandetails-table').DataTable({
        "columnDefs": [
            { "targets": [5, 6], "orderable": false }, // Disable sorting for columns 5, 6
            { "targets": [6], "type": "date", "render": $.fn.dataTable.render.text() } // Specify the column for date filtering
        ]
    });

    // Apply date range filter when the date is changed
    $('#submitDate').on('click', function () {
        var selectedDate = $(this).val();

        // Check if a valid date is selected
        if (selectedDate) {
            // Enable searching through the "Application Date" column
            table.columns(6).search(selectedDate).draw();
        } else {
            // Clear the search when no date is selected
            table.columns(6).search('').draw();
        }
    });

    // Trigger the date filter on page load
    $('#applicationDateFilter').trigger('changeDate');
});
    ...
    <div class="container-xxl position-relative bg-white d-flex p-0">
                ...
                <div class="navbar-nav w-100">
                    ...
                    <div class="nav-item dropdown">
                        ...
                        <div class="dropdown-menu bg-transparent border-0">
                            <a href="loanreport.php" class="dropdown-item active">All loans</a>
        ...
        <div class="content">
            ...
            <div class="align-items-center ms-auto">
                            ...
                            <h2>All loans</h2>
                            <table id="loandetails-table" class="table">
                                <thead>
                                    <tr>
                                        <th>Loan ID</th>
                                        <th>ID Number</th>
                                        ...
                                        <th>Loan Status</th>
                                        <th>Aplication date
                                        <div class="input-group date" id="applicationDateFilter" data-target-input="nearest">
                                            <input type="text" class="form-control datetimepicker-input" data-target="#applicationDateFilter" placeholder="Select Date"> 
                                            <div class="input-group-append" data-target="#applicationDateFilter" data-toggle="datetimepicker">
                                ...
                                <tbody>
                                    <!-- PHP code to fetch data from the farmers table in the database -->
                                        ...
                                        mysqli_close($conn);
                                        ?>
                                        <!-- Display the footer row with total amounts -->
                                        <tfoot>
                                            <tr>
                                                <td colspan="2"></td>
                                                <td>Total Acres: <?php echo $totalAcres; ?></td>
                                                <td>Total Loan Total: <?php echo $totalLoanTotal; ?></td>
                                                <td>Total Interest: <?php echo $totalInterest; ?></td>
                                                <td colspan="2"></td>
                                            </tr>
                                        </tfoot>
                                        </tbody>
                                        ...
                                        <button type="button" class="btn btn-primary" onclick="printCard('loandetails-table')">Print Report</button>
